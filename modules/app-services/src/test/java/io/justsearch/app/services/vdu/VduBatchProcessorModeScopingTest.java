...
class VduBatchProcessorModeScopingTest {
  ...
  void modeTransitionsAreBatchScopedNotPerDocument() throws Exception {
    ...
    verify(vduProcessor, times(3)).process(any(Path.class), any(io.justsearch.core.context.EngineContext.class));
  }
  ...
  void enterVduModeFailureSkipsWholeBatch() throws Exception {
    ...
    verify(vduProcessor, times(1)).enterVduMode();
    verify(vduProcessor, times(0)).process(any(Path.class), any(io.justsearch.core.context.EngineContext.class));
  ...
  void interruptStopsEarlyButStillExitsVduMode() throws Exception {
    ...
    verify(vduProcessor, times(1)).enterVduMode();
    verify(vduProcessor, times(1)).exitVduMode();
    verify(vduProcessor, times(1)).process(any(Path.class), any(io.justsearch.core.context.EngineContext.class));
  }
