...
class AppInstanceLockFailureTest {
  @TempDir Path directory;

  @Test
  void failedChannelCloseRetainsExclusionAndCanRetryWhileOpen() throws Exception {
    var owner = new AppInstanceLock(directory);
    owner.acquire();
    var field = AppInstanceLock.class.getDeclaredField("channel");
    ...
    try {
      ...
      try (var contender = new AppInstanceLock(directory)) {
      ...
      assertFalse(AppInstanceLock.isHeldByThisJvm(directory));
    } finally {
      field.set(owner, real);
      owner.close();
    }
    try (var next = new AppInstanceLock(directory)) { next.acquire(); }
  }
  ...
  void closeFailureAfterNativeCloseCannotBeClearedByANoopRetry() throws Exception {
    ...
    try {
      ...
      try (var contender = new AppInstanceLock(directory)) { assertThrows(IOException.class, contender::acquire); }
      assertFalse(AppInstanceLock.isHeldByThisJvm(directory));
    } finally {
      real.close();
      ...
      var heldField = AppInstanceLock.class.getDeclaredField("HELD_IN_JVM");
      heldField.setAccessible(true);
      Object held = heldField.get(null);
      synchronized (held) {
        ((java.util.Map<?, ?>) held).remove(directory.toRealPath());
      }
    }
  }
  ...
  void failedAcquisitionRetainsReservationWhenChannelCleanupFails() throws Exception {
    try (FileChannel real = FileChannel.open(directory.resolve("app.lock"),
      ...
      try (var opening = mockStatic(FileChannel.class)) {
        opening.when(() -> FileChannel.open(directory.resolve("app.lock"),
        ...
        try {
          ...
          try (var contender = new AppInstanceLock(directory)) { assertThrows(IOException.class, contender::acquire); }
          opening.verify(() -> FileChannel.open(directory.resolve("app.lock"),
  ...
  public static final class Probe {
    public static void main(String[] args) throws Exception {
      ...
      try (var lock = new AppInstanceLock(directory)) {
        lock.acquire();
  ...
  private void metadataFailure(boolean closes) throws Exception {
    try (FileChannel real = FileChannel.open(directory.resolve("app.lock"),
      ...
      try (var opening = mockStatic(FileChannel.class); var owner = new AppInstanceLock(directory)) {
        opening.when(() -> FileChannel.open(directory.resolve("app.lock"),
            StandardOpenOption.CREATE, StandardOpenOption.WRITE)).thenReturn(intercepted);
