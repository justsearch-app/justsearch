...
- *operation outcome after an abrupt death*: a disconnect says nothing about effect, so a
  ...
  that instant plus a small margin answers `expired`, later ones `unknown`, the margin erring
  toward `expired` because a client clock running ahead is the only case that could license a
  duplicate. A malformed, non-v7 or far-future key is refused before anything is accepted; a
  ...
  Engine cannot rule out a prior effect. `history since` advances to the newest evicted
  acceptance, never to the oldest survivor. After a restart, an open row of `interactive`
  survival answers `failed` with reason `interrupted by restart` (its work died with the
  process), a `durable` one stays `accepted` or `running` until its owner resumes it;
