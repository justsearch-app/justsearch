...
| C2 | the versioned SQLite schema ladder; the inert `idempotencyKey` field (an HTTP body field, not proto); the in-memory `OperationHistoryStore` surface that names its own durable swap *(corrected 2026-09-09: the journal has no commit sequence number and its commit-then-`drainPending()` order is effect-before-record, the inverse of 7.5; `putSwitchBuffer` is the fail-closed precedent)* | the operations table and its recoverability row (and `jobs.db` re-classified); acceptance, effect and completion stamps; outcome query; checkpointed resume |
