---
title: "Inference host lane: one port, an in-process adapter, an unshared child, and the device ledger (v4, frozen)"
type: design
status: "FROZEN v4 (2026-09-10) after three independent refute-first review rounds (16, 16 and 6 findings; every finding answered in reviews/). H0 and H1 are decided; H2 is a decision gate; H2b is open. Changes from here follow the lane-F amendment protocol (a dated line in section 0 for a mechanism detail; a decision change is recorded in section 10 with reasoning). Docs grounded at lane-F-A 1465acb7a; code unchanged since 4229f1091, so every file:line is at 4229f1091."
created: 2026-09-10
updated: 2026-09-10
lane: inference host (lane F design.md section 5 names it as the lane after F)
model: fable (design)
related:
  - reviews/                                        # three independent review rounds, their per-finding responses, and design-v3.md
  - theorisation.md                                 # the pre-design record
  - evidence/audits/                                # the four read-only code audits at 4229f1091 (this design's verified facts)
  - ../lane-f-engine-jvm/design.md                  # lane F contract (current text lives on branch worktree-lane-F-A until F merges)
  - ../lane-f-engine-jvm/stages/D1.md               # D1-1, D1-9, D1-12, D1-13, D1-14 (lane branch)
  - ../../decisions/0049-one-engine-jvm-and-the-boundaries-that-survive.md
  - ../../decisions/0004-single-tenant-gpu-policy.md
  - ../../decisions/0046-local-api-trust-boundary.md
  - ../../tempdocs/903-non-nvidia-acceleration-reread.md
  - ../../reference/inference-runtime-register.md   # D-012
---

# Inference host lane: one port, an in-process adapter, an unshared child, and the device ledger (v4, frozen)

## 0. Provenance

**v4 (2026-09-10, freeze).** Round 3 returned five High and one Medium finding against sixteen
and sixteen in rounds 1 and 2, all at named transitions and none touching the direction; that
was the convergence signal set in advance, so v4 closes the six and freezes. The six: an unbound
launch cannot be declared absent from a registry scan and a memory reading (5.3, the PID rule);
adoption must be an `attach` under the drain lock and replacement must follow the predecessor's
confirmed exit (7.2); one fairness cursor per selection class (5.2); a group seals only on the
parent's explicit close, per C1 (4.4); cancelled work is a tombstone retired at seal, not a
watermark (4.4); the ledger charge is resident bytes plus unmaterialised entitlement under a
partition invariant (5.3). `reviews/review-3-response.md` maps each. What remains is
implementation-time proof carried by the stage checklists, not design.

**v3 (2026-09-10).** What v3 changed, and why:
...
- **Engine retention** (C1 `stages/C1.md:1376-1387`, `EngineFutures.java:95-129`; round 3 F4):
  each submission is retained as a child of the C1 task group **before** its send can become
  accepted or ambiguous; it is released on `TERMINATED` or `REFUSED`, or on confirmed host
  death; never on transport detach, never on a timeout. **Sealing is the parent's explicit
  close**, exactly as C1 defines it, never inferred from having no active children; the group's
  retained owner is released only when `sealed AND every accepted child has actually exited`.
  A work id identifies a cancellation group; it is not the lifetime of the local task group.
  Two submissions sharing a work id release independently.
- **Cancellation** (round 3 F5). `cancel(submissionId)` cancels one call. `cancel(workId)` is a
  **group fence** implemented as a cancelled-work tombstone: the host records `cancelled(workId)`
  and refuses **every** later submission carrying that work id, whatever its sequence; queued
  submissions are cancelled; running ones remain accepted work that terminates normally and
  are never turned into pre-admission `REFUSED` records (C1 `:1582-1589`). Tombstones are
  bounded without per-UUID history: one is retired when the group seals (the parent's close,
  after which nothing more can be sent for that work id) or when the client's boot id changes;
  the number of live tombstones is therefore bounded by the number of unsealed work ids, which
  the Engine's C1 admission caps bound. Enqueue and cancel are linearised per work id under one
  lock. A running `run()` is never preempted; a cancelled batch drops its remaining
  sub-batches and then terminates.
...
3. **Fairness cursors, one per selection class** (round 3 F3). Three persistent round-robin
   cursors over lanes, each advanced only by its own class's selections: one for foreground
   candidates, one for aged background candidates when the exception fires, one for background
   candidates when no foreground is pending; FIFO within a lane's producer and client queues.
   A foreground selection therefore cannot reposition the aged cursor, and two continuously
   aged background lanes alternate exceptions. A marked foreground wait that cannot obtain its
   growth entitlement and has no CPU path stays marked and pending; aging stays suspended until
   it is seated or cancelled, which is the correct conservative outcome and is reported.
...
- `BOUND`: the allocation is attributed to an OS identity (child PID plus start time, or the
  host's own session id). **The PID rule** (round 3 F1): the launcher persists the child's PID
  and start time into the intent record immediately after `pb.start()` returns, before any
  other work (`LlamaServerOps.java:1255-1275` registers later today; the install self-test's
  launcher in `app-services/.../ai/runtime/RuntimeActivationService.java` gets the same step).
  A `LAUNCH_PENDING` ticket whose owner crashes before binding is `RETAINED`. It is `RELEASED`
  only when the launch is proved unable to start or continue: the recorded PID and start time
  are confirmed dead; or, when no PID was recorded, a process scan finds no process with the
  intent's executable and a start time after the intent was written. A live match binds the
  ticket instead. A registry miss and a zero memory delta are corroboration only, never the
  release authority. After a host restart, surviving llama entitlements are reconstructed by
  this rule before admission reopens.
...
**Admission** (round 3 F6). Each ticket carries two disjoint balances, `residentBytes` and
`unmaterialisedEntitlementBytes`; its `charge` is their sum, and materialisation moves bytes
from the second to the first without changing the sum. For a session the partition invariant
`resident + sessionRunEntitlement + seatEntitlement = entitlement` holds through load, seat,
persistent growth, arena contraction and retirement. Then:

```
available = total − external − Σ charge(all non-released tickets) − margin
```
...
partial usage is attributed to that ticket's `residentBytes`, not to `external` (round 2
finding 9). Lifecycle state is never a second charge. One authority (one
...
replaced by idle exit**, and adoption and replacement are one locked protocol (round 3 F2):

- With no client for ten minutes the host enters `DRAINING` under its client lock.
- **Adoption is `attach`**, not a bare identity probe: after the identity and config-hash
  checks, the Engine calls `attach`, which under the same client lock as the
  `SERVING -> DRAINING` transition either validates `SERVING`, establishes the lease and cancels
  the idle deadline atomically, or is refused `HOST_DRAINING`. A probe that observed `SERVING`
  confers nothing; only a successful `attach` is an adoption.
- **Replacement follows confirmed exit.** On `HOST_DRAINING`, or on any decision to replace the
  host (a config mismatch under the Engine's own ownership, a version change), the Engine, as
  the host's sole owner, waits for the predecessor's exit within a bound or terminates it by
  identity (`terminateMatched`, the reconciler's existing rule), confirms death by identity, and
  only then spawns a successor. A draining predecessor that still holds a native lease or a
  timed-out retirement (D1-13) is therefore never alive beside a successor that opens admission;
  no second ledger ever exists on the device. The host-lifetime device lock of section 13 is
  needed only when the owner is not the sole party, which is H2b.

The client is a lease with heartbeats; an Engine restart is a client boot-id change: predecessor
submissions terminate on their own, predecessor records retire after termination, and the
successor attaches after the supervisor has restarted the Engine, so in the unshared case the
predecessor Engine is dead by construction. The replacement barrier across live attachments is
H2b's.
...
4. Device grants atomic; the passed bound is per native wait, with the per-request figure
   reported; one fairness cursor **per selection class**; `K` defaults to today.
5. The ledger: allocation classes, a ticket state machine with a persisted launch intent before
   any spawn and the PID recorded immediately after spawn, `RETAINED` for unknown outcomes and
   released only by the PID rule, conservation of the run entitlement into per-seat growth
   tickets under the partition invariant, charge as resident plus unmaterialised entitlement,
   one authority with epoch-tagged readings, in-flight materialisation attributed to its ticket.
...
10. Per-submission terminal records retrieved idempotently and retired by acknowledgement under
    bounds; `cancel(workId)` is a cancelled-work tombstone retired at seal or client boot change;
    sealing is the parent's explicit close per C1; Engine retention releases only on
...
16. Adoption is `attach` under the drain lock; replacement follows the predecessor's confirmed
    exit under the Engine's sole ownership; no second ledger ever exists on a device in H2.
17. **Frozen.** v4 is the contract. Further findings are implementation-time proofs carried by
    the stage checklists; a design change follows the amendment protocol in the status line.
...
across load, seat and release at `K = 2` with the partition invariant asserted at every step;
a `RETAINED` ticket after a simulated crash between intent and registration, reconciled both
ways by the PID rule, including a paused child that allocates after the Engine died); retrieval coverage during chat; binding
...
(two calls sharing a work id released independently; a group not sealed while its parent
still dispatches; lost response recovered by `outcome`; backlog refusal; tombstone refusing a
later-sequence submission and retiring at seal; cancel-before-enqueue; disconnect-during-run;
host death); scheduler fairness with three cursors (two aged lanes alternating under saturating
foreground; the round-3 F3 trace failing on a single cursor).

**H2 rows** additionally: adoption via `attach` refused on a draining host with no lease
established; replacement only after the predecessor's confirmed death, with a draining
predecessor holding a native lease.
