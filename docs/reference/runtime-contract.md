...
| **Internal** | Not for external callers: `/api/debug/*`, the Head↔index-half in-process ports and their `indexing.proto` DTOs, the `contracts/wire` protos, filesystem-only manifest fields (`head.sessionToken`), and the `/infra/capabilities` FE↔Head capability handshake. | No stability, no external audience. |
