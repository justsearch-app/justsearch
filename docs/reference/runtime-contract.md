---
status: stable
description: "JustSearch Runtime Contract — versions, compatibility matrix, stability policy, and the public-contract / reference-client / internal surface classification. The deliberately narrow promise covers the runtime manifest, schema-2 health/status lifecycle subset, and MCP endpoint plus curated tools."
---
...
| `lifecycleSchemaVersion` | Health/status lifecycle subset schema | `LifecycleSnapshotV2.SCHEMA_VERSION` |
...
receipt identity through approval and failure responses. Those additive MCP changes
retain their existing call semantics; the lifecycle schema break is what advances
the umbrella to `0.4.0`.
...
by `assertRuntimeContractCompatible` (currently exactly `0.4.0`). A new runtime-contract version
...
| `0.4.0` | 2026-09-21 | Lifecycle schema `1` → `2`: the health/status lifecycle slots become `api`, `index`, `encoders`, and `generative`, with the same component snapshot in `readiness.engineComponents`. Component states and state-start timestamps replace the legacy capability-state slots. The umbrella bump follows the existing bump-on-break policy. |
...
|---|---|---|
| **Public-contract** | Runtime manifest + its standard transports (`GET /api/runtime/manifest`, the `/.well-known/justsearch/manifest.json` mirror, the manifest SSE stream, the `GET /api/runtime/ready`/`live` probes); the health/status **lifecycle subset** (`GET /api/health`, and the schema-2 lifecycle fields of `GET /api/status`); the **MCP** endpoint (`POST /mcp`) + the curated tool set. | Versioned + deprecation-clocked (above). |
...
- [Health readiness contract](contracts/health-readiness-contract.v1.md) — schema-2 component ownership and diagnostic readiness semantics.
