...
* **Attribution nuance:** the two writes listed above set system properties without a settings-copy marker (`llama.lib.path` and the five ONNX `*.model_path` keys). When debugging “which config won?”, read `/api/debug/effective-config`, whose rows are sourced from the resolver's ordinal chain rather than from any marker.
