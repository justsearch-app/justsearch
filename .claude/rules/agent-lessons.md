...
- **Subagent delivery is type-dependent** (official docs checked 2026-09-08): general-purpose/custom agents load the CLAUDE hierarchy and project rules; Explore/Plan skip it; forks inherit the parent context, including task history. Fresh agents need self-contained briefs. See `docs/reference/contributing/agent-workflow.md` for the versioned matrix and runtime probe. <!-- rule:subagents-no-inheritance -->
- **Session-wide hooks apply inside Claude subagents** (official docs checked 2026-09-08). Actual invocation depends on event matching, client version, trust, and configuration. Earlier no-hook observations are historical, not a current guarantee; verify the specific path with a harmless runtime probe. <!-- rule:parent-hooks-dont-fire-in-subagents -->
